...
def video_process(video_file_path, dst_root_path, ext, fps=-1, size=240):
    ...
    if n_exist_frames >= n_frames:
        print("n_exist_frames {} >= n_frames {}".format(n_exist_frames, n_frames))
        return
    ...
    ffmpeg_cmd = ['ffmpeg', '-i', str(video_file_path), '-vf', vf_param] # , '-vf', vf_param
...
def class_process(class_dir_path, dst_root_path, ext, video_id_df=None, fps=-1, size=240):
    ...
    if video_id_df is not None:
        video_ids_per_class = list(video_id_df[video_id_df['label'] == class_dir_path.stem]['video_id'])
    for video_file_path in sorted(class_dir_path.iterdir()):
        if len(video_ids_per_class) != 0 and video_file_path.stem not in video_ids_per_class:
            continue
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=None,
        type=Path,
        help='List of videos to process')
    parser.add_argument(
    ...
    parser.add_argument(
        '--vid_ext', default='mp4', type=str, help='Extension of videos, avi or mp4.', choices=["avi", "mp4"])
