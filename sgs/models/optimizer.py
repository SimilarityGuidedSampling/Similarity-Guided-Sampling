...
import sgs.utils.lr_policy as lr_policy
...
def construct_optimizer(model, cfg):
    ...
    assert model_param_num == len(non_bn_parameters) + len(
        bn_params
    ), "parameter size does not match: {} + {} != {}".format(
        len(non_bn_parameters), len(bn_params), model_param_num
    )
...
def set_lr(optimizer, new_lr, alpha=1.0):
    """
    Sets the optimizer lr to the specified value.
    Args:
        optimizer (optim): the optimizer using to optimize the current network.
        new_lr (float): the new learning rate to set.
        alpha (float): a factor to control learning after apply lr policy
    """
    for param_group in optimizer.param_groups:
        param_group["lr"] = alpha * new_lr
